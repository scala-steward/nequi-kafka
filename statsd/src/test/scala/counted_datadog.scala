...
object CountedDataDogSpec extends TestSuite with EmbeddedKafkaStreamsAllInOne {
  ...
  def countOne(implicit path: TestPath) = {
    ...
    val out = in.counted(client, pathToTopic(""), tags)
    ...
    assertMatch(m) {
      case CounterMetric(name, value, _, t) if name == pathToTopic("") && value == 1L && t == tags =>
    }
  }
